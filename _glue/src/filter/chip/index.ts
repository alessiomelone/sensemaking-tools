...
class FilterChips extends Component {
  ...
  private readonly addFilterLabel: string;
  private readonly removeFilterLabel: string;
  private focusedChip: number; // Index of the current focused chip in chipArr
  private readonly chipArr: HTMLElement[] = [];
  private readonly stateManager = new FilterStateManager();
  private readonly isRtl = document.documentElement.dir === Strings.RTL;

  constructor(
    root: HTMLElement,
    private readonly data: ChipData,
  ) {
    super(root);
   ...
   */
  private init() {
    this.root.tabIndex = this.isPrimary
      ? TabIndex.TABBABLE
  ...
  private createChip(
    ...
    const chipTextNode = document.createTextNode(
      this.data[filterCategory][filterCategoryItem],
    );
    chipEl.setAttribute(
   ...
   * status
   */
  private render(category: string, categoryItem: string, isActive: boolean) {
    // Find the chip related to this filter, if it exists
    const chipEl = this.root.querySelector<HTMLElement>(
      `[data-glue-filter-category='${category}'][data-glue-filter-category-item='${categoryItem}']`,
    );
    if (this.isPrimary) {
      ...
      // exists
      let rowEl = this.root.querySelector<HTMLElement>(`[role='${Role.ROW}']`);
      if (!rowEl) {
      ...
      rowEl = this.root.querySelector<HTMLElement>(`[role='${Role.ROW}']`);
  ...
  private createCheckmarkIcon() {
    const checkmarkIcon = document.createElementNS(
   ...
   */
  private readonly handleRootKeydown = (evt: KeyboardEvent) => {
    if (
      evt.code === Key.LEFT ||
      evt.code === Key.RIGHT ||
      evt.code === Key.HOME ||
   ...
   */
  private readonly handleChipClick = (evt: Event) => {
    ...
    if (element instanceof HTMLElement) {
      ...
      element.classList.contains(CssClasses.CHIP)
        ? (chipEl = element)
        : (chipEl = element.closest<HTMLElement>(`.${CssClasses.CHIP}`)!);
   ...
   */
  private readonly handleChipKeydown = (evt: KeyboardEvent) => {
    let chipEl: HTMLElement;
    const element = evt.target;
   ...
   */
  private updateChip(chipEl: HTMLElement) {
  ...
  override destroy() {
    ...
    const chipsRowEl = this.root.querySelector<HTMLElement>('div[role=row]');
    if (chipsRowEl) {
