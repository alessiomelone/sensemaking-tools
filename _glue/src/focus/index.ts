 ...
 */
const FOCUSABLE_ELEMENT_SELECTORS: string[] = [
  'input',
  'textarea',
  'select',
  'button',
...
function isElementFocusable(el: Element): boolean {
  return (
    FOCUSABLE_ELEMENT_SELECTORS.filter((selector) => {
 ...
 */
function getFocusableElements(el: Element): HTMLElement[] {
  ...
  const roleBtnElements = el.querySelectorAll<HTMLElement>(
    '[role="button"]' +
      ':not(input)' +
      ':not(a)' +
      ':not(textarea)' +
  ...
  const tabindexElements = el.querySelectorAll<HTMLElement>(
    '[tabindex="0"]' +
      ':not([role="button"])' +
      ':not(input)' +
      ':not(a)' +
  ...
  const mdCheckboxes = el.querySelectorAll<HTMLElement>('md-checkbox');
  const mdRadios = el.querySelectorAll<HTMLElement>('md-radio');
  const mdSwitches = el.querySelectorAll<HTMLElement>('md-switch');
  const mdOutlinedSelects =
    el.querySelectorAll<HTMLElement>('md-outlined-select');
  const mdFilledSelects = el.querySelectorAll<HTMLElement>('md-filled-select');
  const mdOutlinedTextFields = el.querySelectorAll<HTMLElement>(
    'md-outlined-text-field',
  );
  const mdFilledTextFields = el.querySelectorAll<HTMLElement>(
    'md-filled-text-field',
  );
  return [
    ...inputs,
